...
function StopServer {
    mongo --eval "db.shutdownServer({force: true})" --username admin --password p `
        --authenticationDatabase admin --norc admin `
        --tls --host localhost --tlsAllowInvalidCertificates `
        --tlsCAFile C:\Zimmermann\mongodb\key.pem
}
